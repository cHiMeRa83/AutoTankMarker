...
ATM_Settings = ATM_Settings or {
    ...
    autoFocus = false,    -- Deaktiviert wegen Blizzard-UI Blockade
    ...
    tauntAlert = true,    -- Warnung wenn Spott verfehlt / widerstanden wird
    showTankThreatBar = true, -- Tank Aggro-Balken in %
...
local function ShowBannerMessage(message, r, g, b, duration)
    ...
    warnFrame:SetBackdropColor(r * 0.2, g * 0.2, b * 0.2, 0.85)
...
local threatBackdrop = CreateFrame("Frame", nil, threatBar)
...
threatBackdrop:SetBackdropColor(0, 0, 0, 0.8)
threatBackdrop:SetBackdropBorderColor(0.2, 0.2, 0.2, 1.0)

local threatText = threatBar:CreateFontString(nil, "OVERLAY", "GameFontHighlightLarge")
threatText:SetPoint("CENTER", threatBar, "CENTER", 0, 0)
threatText:SetFont("Fonts\\FRIZQT__.TTF", 13, "OUTLINE")
...
threatBar:SetScript("OnUpdate", function(self, elapsed)
    ...
    if threatTimer > 0.15 then
        ...
        if mobUnit then
            ...
            -- Direct Detailed Threat Situation (Player vs Mob)
            local _, _, threatpct = UnitDetailedThreatSituation("player", mobUnit)
            if threatpct then
                pct = math.floor(threatpct)
            else
                -- Fallback für Dungeons: Tank Threat Status auf Mob
                local tankStatus = UnitThreatSituation(tankUnit, mobUnit)
                if tankStatus == 3 then
                    pct = 100 -- Tank hat feste Aggro
                elseif tankStatus == 2 then
                    pct = 80  -- Unsichere Aggro
                elseif tankStatus == 1 then
                    pct = 60  -- Aggro-Verlust droht
                elseif tankStatus == 0 then
                    pct = 30  -- Keine Aggro
                end
            end
            ...
            if pct and pct > 0 then
                ...
                if pct >= 100 then
                    self:SetStatusBarColor(0.0, 0.8, 0.2) -- Grün
                elseif pct >= 80 then
                    self:SetStatusBarColor(1.0, 0.8, 0.0) -- Gelb
                else
                    self:SetStatusBarColor(0.9, 0.1, 0.1) -- Rot
                end
            else
                self:Hide()
            end
...
local function RunTestMode()
    local lang = ATM_Settings.language or "DE"
    isTestingMode = true
    DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[ATM TEST]|r Starte Testmodus inklusive Aggro-Leiste...")
    ...
    threatText:SetText("Tank Aggro: 85% (Testmodus)")
    ...
    testTimer:SetScript("OnUpdate", function(self, elapsed)
        ...
        if step > 2.5 and step < 2.6 then
            ...
            threatText:SetText("Tank Aggro: 100% (Sichere Aggro)")
        ...
        elseif step > 7.5 and step < 7.6 then
            ...
            if not isUnlockedForMoving then threatBar:Hide() end
            isTestingMode = false
            DEFAULT_CHAT_FRAME:AddMessage("|cff00ff00[ATM TEST]|r Test abgeschlossen!")
...
frame:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif event == "COMBAT_LOG_EVENT_UNFILTERED" then
        ...
        -- Defensiv-CD Tracker
        if ATM_Settings.tankDefAlert and currentTankUnit then
            if (subEvent == "SPELL_CAST_SUCCESS" or subEvent == "SPELL_AURA_APPLIED") then
                if (sourceGUID == UnitGUID(currentTankUnit) or (sourceName and sourceName == UnitName(currentTankUnit))) then
...
CreateCheckbox("ATM_CB_TauntAlert", "Warnung wenn Spott des Tanks verfehlt", -220, "tauntAlert")

-- BUTTON: TESTMODUS STARTEN
local btnTest = CreateFrame("Button", "ATM_Btn_RunTest", optionsPanel, "UIPanelButtonTemplate")
btnTest:SetPoint("TOPLEFT", 16, -260)
...
-- CHECKBOX: AGGRO-LEISTE VERSCHIEBEN / EINBLENDEN
local cbMove = CreateFrame("CheckButton", "ATM_CB_MoveThreatBar", optionsPanel, "InterfaceOptionsCheckButtonTemplate")
cbMove:SetPoint("TOPLEFT", 185, -260)
_G[cbMove:GetName() .. "Text"]:SetText("Aggro-Leiste freigeben")
cbMove:SetScript("OnClick", function(self)
    ...
    if isUnlockedForMoving then
        ...
        threatBar:SetValue(85)
        threatBar:SetStatusBarColor(1.0, 0.8, 0.0)
        threatText:SetText("Aggro-Leiste (Ziehen zum Verschieben)")
    else
        threatBar:Hide()
    end
end)
...
iconHeader:SetPoint("TOPLEFT", 16, -375)
...
iconDropdown:SetPoint("TOPLEFT", 6, -395)
